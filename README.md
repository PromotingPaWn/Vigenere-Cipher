...
Vigenère Cipher Implementation (C)이 프로젝트는 다중 치환 암호의 일종인 비즈네르 암호(Vigenère Cipher)를 C 언어로 구현한 보안 프로그래밍 예제입니다.

1. [프로젝트 개요]
   -비즈네르 암호는 키워드를 사용하여 평문을 암호화하는 방식으로, 단일 치환 암호의 단점인 빈도 분석 공격을 보완한 알고리즘입니다.
   -본 프로그램은 26x26 비즈네르 표를 메모리에 생성하여 암호화 로직을 수행합니다.

2. [주요 기능 및 특징]
   -비즈네르 표 자동 생성: 프로그램 실행 시 2차원 배열(char vigenere[26][26])을 활용하여 암호화용 알파벳 표를 동적으로 생성합니다.
   -안정적인 입력 처리: fgets를 사용하여 버퍼 오버플로우를 방지하고, 유효하지 않은 입력(공백, 특수문자 등)에 대한 예외 처리가 포함되어 있습니다.
...
4. [암호화 원리]
   ...
   -Key 처리: 키워드의 공백을 제거하고 순수 알파벳으로만 구성합니다.
   -Mapping: 평문 알파벳 $P$와 키워드 알파벳 $K$를 비즈네르 표의 인덱스로 매핑합니다.
   -Result: vigenere[K-97][P-97] 위치의 문자를 출력합니다.
